...
class command_aptget(HoneyPotCommand):
    ...
    def do_install(self, *args):
        ...
        for y in [re.sub('[^A-Za-z0-9]', '', x) for x in self.args[1:]]:
            packages[y] = {
                'version':      '%d.%d-%d' % (random.choice((0, 1)), random.randint(1, 40), random.randint(1, 10)),
        ...
        self.writeln('0 upgraded, %d newly installed, 0 to remove and 259 not upgraded.' % len(packages))
        self.writeln('Need to get %s.2kB of archives.' % (totalsize))
        self.writeln('After this operation, %skB of additional disk space will be used.' % (totalsize * 2.2,))
        i = 1
        for p in packages:
            self.writeln('Get:%d http://ftp.debian.org stable/main %s %s [%s.2kB]' % (i, p, packages[p]['version'], packages[p]['size']))
            i += 1
        ...
        yield self.sleep(1, 2)
        for p in packages:
            self.writeln('Unpacking %s (from .../archives/%s_%s_i386.deb) ...' % (p, p, packages[p]['version']))
            yield self.sleep(1, 2)
        ...
        for p in packages:
            ...
            self.honeypot.commands['/usr/bin/%s' % p] = command_faked_package_class_factory.getCommand(p)
            yield self.sleep(2)
        self.exit()
